...
class VoiceLoss(nn.Module):
    ...
    def __init__( # pylint: disable=dangerous-default-value
        ...
        max_perceptual_weight=0.15,
        perceptual_start_epoch=5,
        perceptual_ramp_epochs=10,
    ):
        super().__init__()
        self.l1 = nn.L1Loss()
        self.mse = nn.MSELoss()
    ...
    def forward(
        ...
        if target_audio is not None:
            ...
            stft_loss = sum(self.l1(p, t) for p, t in zip(mr_pred, mr_tgt)) / len(
                self.fft_sizes
            )
        ...
        if (
            self.use_perceptual_loss and epoch > self.perc_start_epoch
        ):  # Start after initial training
        ...
        if self.use_perceptual_loss and epoch >= self.perc_start_epoch:
            ...
            perc_weight = min(
                self.max_perc_weight, max(0.0, prog * self.max_perc_weight)
            )
        ...
        loss_components = {
            ...
            "perceptual_loss": (
                ...
                if isinstance(perceptual_loss, torch.Tensor)
                else 0.0
            ),
        ...
        if perceptual_components:
            for key, value in perceptual_components.items():
                loss_components[f"perceptual_{key}"] = (
...
class TrainingLogger:
    ...
    def log_audio(self, audio, sample_rate, caption, step, is_reference=False):
        ...
        if self.use_wandb:
            if is_reference:
                wandb.log(
                    {
                        f"Reference Audio Epoch {step}": wandb.Audio(
    ...
    def log_spectrogram(self, spec_img, caption, step, is_reference=False):
        ...
        # Allow passing in either a numpy image (spectrogram matrix) **or**
        # a pre-made Matplotlib Figure.
    ...
    def log_voice_drift(self, drift_metrics, step=None):
        ...
        if self.use_wandb:
            ...
            if "top_changed_dims" in drift_metrics:
                ...
                for i, (change, dim_idx) in enumerate(
                    zip(top_dims.values, top_dims.indices)
                ):
...
class VoiceEmbedding:
    ...
    def get_timbre(self, length=None):
        ...
        """
        if length is None:
            return self.base_voice[0, : self.embedding_size // 2] # pylint: disable=unsubscriptable-object
        else:
            voice = self.get_for_length(length)
            return voice[0, 0, : self.embedding_size // 2] # pylint: disable=unsubscriptable-object
    ...
    def check_health(self, epoch=0, warning_threshold=0.3):
        ...
        with torch.no_grad():
            ...
            timbre_norms = torch.norm(
                self.voice_embed[:, 0, : self.embedding_size // 2], dim=1
            )
    ...
    def apply_length_smoothing(self, weight=0.2):
        ...
        with torch.no_grad():
            ...
            for i in range(1, self.max_phoneme_len):
                smoothed[i] = ( # pylint: disable=unsupported-assignment-operation
                    weight * self.voice_embed[i - 1]
                    + (1 - weight) * self.voice_embed[i]
                )
    ...
    def save(self, path):
        ...
        voice_embedding = self.voice_embed.detach().cpu() # pylint: disable=not-callable
        state_dict = {
...
def calculate_voice_drift(original_voice, voice_embedding, device):
    """
    Calculate voice drift metrics between the original reference voice and the
    current voice embedding.
    ...
    with torch.no_grad():
        ...
        trained_avg = (
            voice_embedding.voice_embed.mean(dim=0).squeeze().to(device)
        )  # [256]

        # Calculate various similarity metrics
        cosine_sim = F.cosine_similarity( # pylint: disable=not-callable
            original_avg.unsqueeze(0), trained_avg.unsqueeze(0) # pylint: disable=not-callable
        ).item()
...
def generate_with_custom_voice(text, voice_path, output_file="output.wav"):
    """
    Generate speech using a custom voice with the Kokoro TTS model.
    
    Args:
        text: The input text to synthesize into speech
        voice_path: Path to the voice embedding file (.pt format)
        output_file: Path where the generated audio will be saved (default: output.wav)
...
def generate_with_standard_voice(text, output_file="output.wav"):
    """
    Generate speech using a standard Kokoro voice (af_heart).
...
def calculate_audio_similarity(
    voice_embedding, target_audio_path, text, sr=24000
):
    """
    ...
    try:
        ...
        generate_with_custom_voice(
            text, temp_file_reference.name, temp_file_generated.name
        )
        ...
        spectral_conv = np.linalg.norm(
            generated_mel_db - target_mel_db, "fro"
        ) / np.linalg.norm(target_mel_db, "fro")
        ...
        try:
            ...
            if len(generated_f0_nonzero) > 0 and len(target_f0_nonzero) > 0:
                f0_diff = abs(
                    np.mean(generated_f0_nonzero) - np.mean(target_f0_nonzero)
                )
            ...
            traceback.print_exc()
            print(f"Error calculating F0 similarity: {e}")
            metrics["f0_difference_hz"] = float("inf")
            metrics["f0_similarity"] = 0.0

        # 6. Spectral centroid similarity
        generated_centroid = librosa.feature.spectral_centroid(
            y=generated_audio, sr=sr
        )[0]
...
def calculate_training_similarity_loss(pred_mel, target_mel, device):
    ...
    spectral_conv = torch.norm(pred_mel - target_mel, p="fro") / (
        torch.norm(target_mel, p="fro") + 1e-8
    )
    ...
    for i in range(pred_mel.shape[1]):  # For each frequency bin
        ...
        pred_freq_norm = (pred_freq - pred_freq.mean(dim=1, keepdim=True)) / (
            pred_freq.std(dim=1, keepdim=True) + 1e-8
        )
        target_freq_norm = (target_freq - target_freq.mean(dim=1, keepdim=True)) / (
            target_freq.std(dim=1, keepdim=True) + 1e-8
        )
    ...
    freq_bins = (
        torch.arange(pred_mel.shape[1], dtype=torch.float32, device=device)
        .unsqueeze(0)
        .unsqueeze(-1)
    )

    pred_centroid = (freq_bins * pred_mel).sum(dim=1) / (pred_mel.sum(dim=1) + 1e-8)
    target_centroid = (freq_bins * target_mel).sum(dim=1) / (
        target_mel.sum(dim=1) + 1e-8
    )
...
def interpret_audio_similarity(metrics):
    ...
    if perceptual > 0.85:
        interpretation.append(
            " Excellent perceptual similarity - voices sound very similar"
        )
    elif perceptual > 0.70:
        interpretation.append(
            " Good perceptual similarity - voices are recognizably similar"
        )
