...
class DatasetUploadTUI:
    ...
    def select_dataset_directory(self) -> Optional[Path]:
        ...
        while True:
            ...
            if not dataset_dir.exists():
                self.console.print(f"❌ [red]Directory does not exist: {dataset_dir}[/red]")
                if not Confirm.ask("Try again?"):
                    return None
                continue
            
            if not dataset_dir.is_dir():
                self.console.print(f"❌ [red]Path is not a directory: {dataset_dir}[/red]")
                if not Confirm.ask("Try again?"):
                    return None
                continue
            
            # Quick check for dataset structure
            has_splits = any((dataset_dir / split).exists() for split in ["train", "validation", "test"])
            
            if not has_splits:
                self.console.print(f"⚠️ [yellow]Warning: No train/validation/test directories found in {dataset_dir}[/yellow]")
    ...
    def show_upload_summary(self, dataset_dir: Path, config: dict, validation: dict):
        ...
        if validation["warnings"]:
            self.console.print(f"⚠️ [yellow]{len(validation['warnings'])} warnings[/yellow]")
        
        if validation["errors"]:
            self.console.print(f"❌ [red]{len(validation['errors'])} errors[/red]")
